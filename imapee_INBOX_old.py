...
EMAIL_SERVER_INBOX= os.getenv("EMAIL_SERVER_INBOX")

print(f"{EMAIL_ACCOUNT} | {PASSWORD}")
...
with MailBox(EMAIL_SERVER_INBOX).login(EMAIL_ACCOUNT, PASSWORD) as mailbox:
    ...
    # for msg in mailbox.fetch(AND(from_='Mail Delivery'), mark_seen=False):
        ...
        if email_sender not in set_emails:
            valid_email = my_utils.validate_email_format(email_sender)
            if valid_email != False:
                if v:
                    print('email_sender', type(valid_email), '------', valid_email)
        ...
        if len(emails_cc) > 0:
            for contact in emails_cc:
                ...
                valid_email = my_utils.validate_email_format(email_cc)
                if valid_email != False:
                    if v:
                        print('email_sender', type(valid_email), '------', valid_email)
                    set_emails.add(valid_email)
        ...
        for re_match in re.finditer(EMAIL_REGEX, msg.html.lower()):
            ...
            valid_email = my_utils.validate_email_format(new_email_from_html)
            if valid_email != False:
                if v:
                    print('email_sender', type(valid_email), '------', valid_email)
                set_emails.add(valid_email)
        ...
        for re_match in re.finditer(EMAIL_REGEX, msg.text.lower()):
            ...
            valid_email = my_utils.validate_email_format(new_email_from_text)
            if valid_email != False:
                if v:
                    print('email_sender', type(valid_email), '------', valid_email)
                set_emails.add(valid_email)
...
for ue in set_emails:

    count_emails += 1

    print(count_emails, '\t', ue)
