...
using_vendor_uids = [f"{x.domain}-{x.vendor}" for x in select_all_records(DB_BTOB, 'using_vendor')]
...
blacklist_emails = list(vendors_domains) + list(socials_domains) + [
    ...
    'hubilo-webinar.com',
    'mailchimpapp.net',
    'c.constantcontact.com',
    'treasurydragons.org',
    'email.infragistics.com',
    'e-nqa.com',
...
count_emails_processed = 0
count_errors = 0
count_mark_as_read = 0
count_deleted = 0
count_total = 0
count_using_vendor = 0
count_new_emails = 0
                ...
                if email_domain not in vendors_domains:
                    ...
                    if valid_email != False and valid_email not in set_emails:
                        ...
                        # logger.info(f"ℹ️\tADDED {email} to set_emails")
                                ...
                                print(f"ℹ️\t{email} in existing_emails")
                    ...
                    print(f"❌\t{email} in vendors_domains")

            else:

                print(f"❌\t{email} in blacklist_emails")
...
with MailBox(EMAIL_SERVER).login(EMAIL_ACCOUNT, PASSWORD) as mailbox:
    ...
    for msg in mailbox.fetch(mark_seen=False, reverse=False, bulk=True): # get all emails from most recent without changing read status
        ...
        count_emails_processed += 1

        to_delete = False

        if count_emails_processed < 10000000: # FOR TESTING ONLY
            ...
            print(f"\n\n\n========= {count_emails_processed} / {email_sender}: {msg.subject}\n")
            ...
            # PROCESS EMAILS
                ...
                # ADD to btob.using_vendor

                if vendor_domain not in [None, 'hubilo.com']: # avoid adding replies with my email

                    using_vendor_uid = f"{email_sender_domain}-{vendor_domain}"
                    if using_vendor_uid not in using_vendor_uids:

                        if len(url_reg) > 0:
                            url_reg = list(url_reg)[0]
                        else:
                            url_reg = None
                        ...
                        create_record(DB_BTOB, 'using_vendor', 
                            ...
                            'domain': email_sender_domain,
                            'vendor': vendor_domain,
                            'url_org': email_sender,
                            'src': f"imapee_HU_WebReg {ts_db}",
                            'url_reg': url_reg,
                            'created': ts_db,
                            })

                        using_vendor_uids.append(using_vendor_uid)

                        count_using_vendor += 1
                ...
                print(f"\n\nimapee_HU_WebReg.email_data = {email_data}")


                # Add emails to DB / 2023-10-10 16:19 workaround to add the sender's email too
                for clean_email in clean_emails_list:
            ...
            print(f"{count_emails_processed} max_emails reached", end='\r', flush=True)
...
if __name__ == '__main__':
    ...
    print()
    print('-------------------------------')
    print(f"count_emails_processed =\t{count_emails_processed:,}")
    print(f"count_new_emails =\t{count_new_emails}")
    print(f"count_using_vendor =\t{count_using_vendor}")
